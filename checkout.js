...
<div class="flex smalls:flex-row flex-col gap-4 w-full">
    <div class="flex gap-6 w-full">
        <div class="md:w-[7rem]  md:h-[6rem] min-h-28 h-28 w-[7rem] flex justify-center items-center  bg-contain"><!-- image container -->
            ...
            <div class="w-full text-nowrap flex  gap-2"><p class="flex gap-2 items-center">Quantity: <span class="quantity-number${matching.id}">${cartItem.quantity}</span> <span class="hidden saveInput-${matching.id}" ><input type="text" class="border-2 w-9 rounded-md" placeholder=""><span class="ml-2 save-link cursor-pointer" data-product-id="${matching.id} ">save</span></span> <span class="text-blue-500 flex gap-4 " > <span class="check-out-update cursor-pointer hover:text-orange-500" data-product-id="${matching.id} ">Update</span> <span class="cursor-pointer hover:text-blue-400 delete-button delete-btn-${matching.id} " data-product-id="${matching.id}">Delete</span></span></div>
            </p></div>
    </div>
...
document.querySelectorAll('.check-out-update').forEach((update) =>{
    update.addEventListener('click', ()=>{
        ...
        const quantityUpdate = document.querySelector(`.quantity-number${updateId}`)
        if(del){
            const delId = del.dataset.productId.trim();
      if(updateId === delId){
        update.classList.toggle('hidden');
        saveInput.classList.toggle('hidden');
        quantityUpdate.classList.toggle('hidden');
      }
    }
      else {
...
document.querySelectorAll('.save-link').forEach((save) =>{
    save.addEventListener('click', ()=>{
        const saveId = save.dataset.productId;
        const saveInput = document.querySelector(`.saveInput-${saveId}`)
        const quantityUpdate = document.querySelector(`.quantity-number${saveId}`)
        saveInput.classList.toggle('hidden');
        quantityUpdate.classList.toggle('hidden');
